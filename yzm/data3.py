...
# 验证码图片的存放路径
CAPTCHA_IMAGE_PATH = 'F:/python/yzm/train/'
# 用于模型测试的验证码图片的存放路径，它里面的验证码图片作为测试集
TEST_IMAGE_PATH = 'F:/python/yzm/test/'
# 用于模型测试的验证码图片的个数，从生成的验证码图片中取出来放入测试集中
TEST_IMAGE_NUMBER = 50


# 生成验证码图片，4位的十进制数字可以有10000种验证码
def get_image(charSet=CHAR_SET, charSetLen=CHAR_SET_LEN, captchaImgPath=CAPTCHA_IMAGE_PATH):
    k = 0
    total = 1
...
if __name__ == '__main__':
    get_image(CHAR_SET, CHAR_SET_LEN, CAPTCHA_IMAGE_PATH)
    # prepare_test_set()
    # sys.stdout.write("\nFinished")
    # sys.stdout.flush()
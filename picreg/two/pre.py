...
import numpy as np
import os
from two import train
import time
import cv2
MOVING_AVERAGE_DECAY = 0.99
EVAL_INTERVAL_SECS = 10
image_size = 128
img = cv2.imread("./images/0/Black_Footed_Albatross_0001_796111.jpg")
...
with tf.Session() as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        for i in range(1):
            ...
            cv2.putText(img, "class=%s"%ss, (20, 20), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 0), 1)
            cv2.imshow("a", img)
            cv2.waitKey(0)
    else:
